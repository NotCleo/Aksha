...
void spi_init() {
    ...
    fprintf(stderr, "[HW] SPI FIFO depth=%u | reg-read=%.2fus | byte=%.1fus (SCK approx %.0f kHz)\n",
            spi_fifo_depth, rd_us, byte_us, 8000.0 / byte_us);
}
...
// Fast bulk read for use inside an already-CS-asserted burst: queue a whole
// FIFO-worth of dummy bytes into the TX FIFO, wait until the RX FIFO holds
// the same count (occupancy reg 0x78 reads count-1), then drain it blind.
// ~2 register accesses per byte instead of ~5-7 -> the 5MP JPEG readout
// drops from ~25s to a few seconds. Byte order/count identical to the
// one-at-a-time path; falls back to it if the core has no FIFOs.
void spi_read_burst(uint8_t *dst, uint32_t n) {
    ...
    while (n) {
        uint32_t chunk = n < spi_fifo_depth ? n : spi_fifo_depth;
        for (uint32_t i = 0; i < chunk; i++) SPI_REG(0x68) = 0x00;
        int timeout = 0;
        while ((SPI_REG(0x64) & 0x01) || ((SPI_REG(0x78) & 0xFF) != chunk - 1)) {
            if (++timeout > 200000) break;   // bounded, like spi_transfer
        }
        for (uint32_t i = 0; i < chunk; i++) dst[i] = (uint8_t)SPI_REG(0x6C);
        dst += chunk;
        n -= chunk;
    }
}
...
void i2c_write_reg16(uint16_t reg, uint8_t val) {
    I2C_REG(0x40) = 0x0A;
    for (int i = 0; i < 8; i++) (void)I2C_REG(0x104);  // ~25us reset settle
    I2C_REG(0x100) = 0x01;
    ...
    // Wait for completion by POLLING the status register (TX FIFO drained
    // AND bus idle) instead of usleep(): this kernel rounds usleep() up to
    // a ~10ms scheduler tick, so the two blind sleeps that used to sit in
    // this function cost ~20ms per register write - ~16 of the 26 seconds
    // of a full capture. The real transaction is ~450us at 100kHz SCL
    // (fabric-confirmed: axi_iic IIC_FREQ_KHZ=100), ~140 poll reads.
    int timeout = 0;
    for (;;) {
...
int jpeg_capture(const uint16_t (*size_arr)[2], const char *size_desc, uint8_t quality) {
    double t_cfg = now_ms();
    ...
    if (v3818 != 0xa8 || v4407 != quality) {
        printf("  [WARN] JPEG config readback mismatch - continuing, but if the capture\n");
        printf("  fails, power-cycle the board and rerun.\n");
    }
    ...
    usleep(1500000); // let auto-exposure/AWB converge on live frames first
    ...
    if (len <= 1024 || len >= 0x7FFFF0) {
        printf("[FAIL] No plausible JPEG in FIFO (len=%u). If len is 8388607 the SPI bus\n", len);
        printf("read all-0xFF (connection dropped) - power-cycle and reseat the module.\n");
        return -1;
    }
    ...
    fprintf(stderr, "[TIME] config=%.2fs settle=%.2fs capture=%.2fs readout=%.2fs (%u bytes, %.1fus/byte)\n",
    ...
    for (c = 0; c < 3 && !found; c++) {
        ...
        if (found && soi > 64) found = 0; // the frame must begin near the front
    }

    if (!found) {
        printf("[WARN] No JPEG start marker in any readout alignment - saving the raw\n");
        printf("stream to 'fifo_dump.bin'. scp it to the laptop for byte-level analysis.\n");
...
int main(int argc, char **argv) {
    ...
    if (argc >= 2) {
        ...
        jpeg_mode = 1;
        if (argc >= 3) {
            if      (!strcmp(argv[2], "320"))  { size_arr = ov5642_320x240;   size_desc = "320x240"; }
            else if (!strcmp(argv[2], "640"))  { size_arr = ov5642_640x480;   size_desc = "640x480"; }
            else if (!strcmp(argv[2], "1024")) { size_arr = ov5642_1024x768;  size_desc = "1024x768"; }
            else if (!strcmp(argv[2], "1280")) { size_arr = ov5642_1280x960;  size_desc = "1280x960"; }
            else if (!strcmp(argv[2], "1600")) { size_arr = ov5642_1600x1200; size_desc = "1600x1200"; }
            else if (!strcmp(argv[2], "2048")) { size_arr = ov5642_2048x1536; size_desc = "2048x1536"; }
            else if (!strcmp(argv[2], "2592")) { /* default */ }
            else { printf("Unknown size '%s' (use 320/640/1024/1280/1600/2048/2592)\n", argv[2]); return -1; }
        }
        if (argc >= 4) {
            int q = atoi(argv[3]);
            if (q < 2) q = 2;
    ...
    if (id_h != 0x56 || id_l != 0x42) {
        printf("FAIL - sensor not responding on I2C.\n");
        return -1;
    }
    printf("PASS\n");

    if (jpeg_mode)
        return jpeg_capture(size_arr, size_desc, quality);
