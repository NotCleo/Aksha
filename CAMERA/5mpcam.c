...
uint8_t i2c_read_reg16(uint16_t reg) {
    ...
    int timeout = 0;
    while ((I2C_REG(0x114) & 0xFF) == 0) {  // RX_FIFO_OCY: 0 = empty
        usleep(100);
        if (++timeout > 1000) break;         // ~100ms cap, don't hang forever
    }
    usleep(2000);
...
int jpeg_capture(const uint16_t (*size_arr)[2], const char *size_desc, uint8_t quality) {
    ...
    for (uint32_t i = 0; i < len + 2; i++) {
        s[i] = spi_transfer(0x00);
        if ((i & 0x3FFFF) == 0x3FFFF) { printf("  ... %u KB\n", (i + 1) / 1024); fflush(stdout); }
    }
    SPI_REG(0x70) = 0xFFFFFFFF;
